...
use ATM
select [User].name, cardNum
from [User] inner join UserCard on [User].userId = UserCard.userID

select cardNum,balance,name
from Card inner join CardType on CardType.cardTypeID = Card.cardTypeID

select T.cardNum,T.transId, T.transDate,T.amount
from Card left join [Transaction] T on T.cardNum = Card.cardNum

select U.userId, U.name, U.phoneNum, U.city, T.transId, T.transDate, T.cardNum, T.amount
from [User] U full outer join UserCard UC on U.userId = UC.userID
full outer join [Transaction] T on T.cardNum = UC.cardNum

select *
from [UserCard] UC right join [User] U on U.userId=UC.userID
where UC.cardNum is null

select UC.cardNum,[User].name
from [User] left join UserCard UC on [User].userId = UC.userID
...
from [User] U left join UserCard UC on U.userId = UC.userID
left join Card C on UC.cardNum = C.cardNum
left join CardType CT on C.cardTypeID = CT.cardTypeID
where C.cardNum is not null and datediff(m,C.expireDate,GETDATE())<=3