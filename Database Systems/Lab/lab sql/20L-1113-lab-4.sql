...
-- Task 1 starts here
-- What is maximum, minimum, average and standard deviation of ages of the users? (Search Standard Dev function)
use twitter
...
-- Task 2 starts here
-- Give name of the user who has the highest number of followers.
use twitter
select top 2 F.FollowedUserName as UserName, count(*) as NoOfFollowers
from Following F
...
use twitter
select F.FollowedUserName as UserName, count(*) as NoOfFollowers
from Following F
group by F.FollowedUserName
go
...
use twitter
select Hashtag,UserName,count(UserName) as noOfTimes
...
-- Task 6 starts here
-- Find the users who have never used the hashtag #Census.
use twitter
select distinct T.UserName
from Tweets T
where T.Text not like '%#Census%'
go
...
-- Task 8 starts here
-- List all the usernames that have never been followed. Using Exist Clause.
use twitter
select U.UserName
from [Users] U
where exists(
...
-- Task 9 starts here
-- Find the most common interest of users. (The interest with largest number of users). Also find the least common interest.
use twitter
select U.InterestID, count(*) as Countinterest
...
use twitter
select U.Country as country, count(*)
...
use twitter
select count(*)
from [Users] U full outer join Tweets T on U.UserName = T.UserName
group by U.UserName
order by U.UserName
go
...
use twitter
select top 1 I.InterestID,T.Description, count(*) as CountInterest
from [dbo].[UserInterests] as I full outer join [Interests] T on I.InterestID = T.InterestID
group by I.InterestID,T.Description
order by count(*) desc
go